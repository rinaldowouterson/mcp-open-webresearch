...
describe("Smart Fetch Modes (Local Integration)", () => {
    ...
    test("browserMode: true sends Chrome-like User-Agent", async () => {
        ...
        const endpoint = await mockServer.forGet("/browser-test").thenReply(200, "ok");
        
        const response = await smartFetch(mockServer.url + "/browser-test", { browserMode: true });
    ...
    test("browserMode: false sends standard User-Agent (NOT Chrome)", async () => {
        const smartFetch = await loadSmartFetchWithConfig(true);
        const endpoint = await mockServer.forGet("/standard-test").thenReply(200, "ok");
        
        const response = await smartFetch(mockServer.url + "/standard-test", { browserMode: false });
    ...
    test("handles HTTP errors correctly", async () => {
        ...
        await expect(smartFetch(mockServer.url + "/error")).rejects.toThrow("HTTP Error: 404");
    });
});
