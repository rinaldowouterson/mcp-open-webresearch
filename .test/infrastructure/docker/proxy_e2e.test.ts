...
describe("Proxy E2E Tests", () => {
  ...
  afterEach(async () => {
    ...
    const { cleanBrowserSession } =
      await import("../../../src/infrastructure/visit_page/visit.js");
  ...
  it("should route traffic through an HTTP proxy", async () => {
    ...
    const targetRequest = seenRequests.find(
      (r) => r.url === TEST_URL_HTTP || r.url === TEST_URL_HTTP.slice(0, -1),
    );
      ...
      socksServer.listen(socksPort, "127.0.0.1", async () => {
        ...
        try {
          ...
          if (!authSuccess) {
            reject(
              new Error(
                "Page visited but SOCKS proxy authentication NOT performed",
              ),
            );
          }
          if (!proxyConnected) {
